...
async function fetchTranscript() {
  ...
  for (const segment of segments) {
    // For transcript-segment-view-model, get text content excluding timestamp
    // Try known text selectors first
    ...
    // Fallback: extract full text minus any timestamp-looking prefix
    const fullText = segment.textContent?.trim();
    if (fullText) {
      // Remove timestamp pattern like "0:00" or "12:34" at the start
      const withoutTimestamp = fullText.replace(/^\d+:\d{2}\s*/, '').trim();
      if (withoutTimestamp) {
        transcript += withoutTimestamp + ' ';
      }
    }
  }
